...
export const sendWelcomeEmail = async ({ email, name, intro }: WelcomeEmailData) => {
    ...
    const mailOptions = {
        from: `"jaywantpatel" <jaywantpatel467@gmail.com>`,
        to: email,
        subject: 'Welcome to Stracker - your stock market tookkit is ready!',
        text: 'Thanks for joining Stracker',
        html: htmlTemplate,
    }
...
export const sendNewsSummaryEmail = async ({email, date, newsContent}: {email: string; date: string; newsContent: string}): Promise<void> => {
    ...
    const mailOptions = {
        ...
        text: "Today's market news summary from Stracker",
        html: htmlTemplate,
    };
